...
TEST(UtilsNumberTests, String2Number)
{
    ...
    EXPECT_EQ(s2num<int16_t>("32767"), 32767);
    EXPECT_EQ(s2num<int16_t>("-32768"), -32768);
    EXPECT_EQ(s2num<uint16_t>("65535"), 65535);

    EXPECT_EQ(s2num<int32_t>("2147483647"), 2147483647);
    EXPECT_EQ(s2num<int32_t>("-2147483648"), -2147483648);
    EXPECT_EQ(s2num<uint32_t>("4294967295"), 4294967295);

    EXPECT_EQ(s2num<int64_t>("9223372036854775807"), 9223372036854775807);
    EXPECT_EQ(s2num<int64_t>("-9223372036854775808"), -9'223'372'036'854'775'807LL - 1);
    EXPECT_EQ(s2num<uint64_t>("18446744073709551615"), 18446744073709551615ULL);

    EXPECT_EQ(s2num<int>("1234567890"), 1234567890);
...
TEST(UtilsNumberTests, String2NumberEdgeCases)
{
    ...
    EXPECT_EQ(s2num<int>("2147483648", -1), -1);  // out of range for int, returns default value
}
...
TEST(UtilsNumberTests, Digits)
{
    ...
    EXPECT_EQ(digits(-12345), 6); // includes the '-' sign
    EXPECT_EQ(digits(1234567890), 10);
    EXPECT_EQ(digits(4294967295), 10);
